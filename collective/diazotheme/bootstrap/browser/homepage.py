...
#        url = "http://list.mp3.baidu.com/index.html"
    ...
    def _extract_data(self, data, tag=None, cssid=None,
                      cssclass=None, attrs=None, regexp=None, index=0):
        """
        ...
        if cssid:
            ...
            if isinstance(attrs, unicode):
                try:
                    ...
                    pass
            searchconstrain = SoupStrainer(
                tag, attrs={attrs: re.compile(regexp)})
...
# n output js
    def outputjs(self):
        ...
        out = """$(document).ready(function(){rolltext(".%(mid)s");});""" % dict(
            mid=cssid)
        return out
####################
# slide bar
...
#        collection_path = target_collection
        ...
        if bool(ctobj):
                # pass on batching hints to the catalog
            braindata = ctobj[0].getObject().queryCatalog(
                batch=True, b_size=limit, sort_on="created")
